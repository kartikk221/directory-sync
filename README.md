...
Declarative Server filters are included in that manifest response and enforced by Mirrors before filesystem mutations enter the transfer queue. Ignored paths therefore produce no upload request or per-path filter log. Function filters remain Server-only because executable code is never sent over the network; the Server silently rejects any mutation they exclude.

Public logs are limited to `MANIFEST`, `WEBSOCKET`, `UPLOAD`, and `DOWNLOAD`. For every file transferred, its source emits `UPLOAD` start and completion logs while its destination emits matching `DOWNLOAD` start and completion logs. Manifest exchanges and WebSocket connections, reconnections, and disconnections are also logged; local filesystem bookkeeping, retries, filters, and hard-sync passes are intentionally silent. Operational failures continue to use the `error` event.
...
File transfers are streamed with backpressure and default to 100 concurrent files through `queue.max_concurrent`. Complete reconciliation uses a fixed worker set instead of allocating one promise per file. Live changes are coalesced to the newest pending mutation per URI, with at most 10,000 distinct URIs retained by default through `queue.max_pending`; exceeding that bound discards the inbox and schedules one authoritative manifest reconciliation. Directory mutations remain ordering barriers so parent/child changes cannot race. Manifest hashing is separately limited to at most four files by default through `hashing.max_concurrent`, avoiding unbounded random disk reads. Hashes are reused only in memory while a process is running and only while both file size and `mtime` remain unchanged; no hash or manifest cache is persisted.
