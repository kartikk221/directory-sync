...
Routine public logs use `MANIFEST`, `WEBSOCKET`, `UPLOAD`, `DOWNLOAD`, `CREATE`, and `DELETE`. For every file transferred, its source emits `UPLOAD` start and completion logs while its destination emits matching `DOWNLOAD` start and completion logs. `CREATE` and `DELETE` describe files and directories actually changed by synchronization; a recursive directory deletion emits one parent log instead of one log per descendant. Manifest exchanges, completed manifest-sync passes, and WebSocket connections, reconnections, and disconnections are also logged. Durations use each process's local elapsed time and end only after the stream or atomic filesystem mutation completes. Exceptional conditions such as `SIZE_LIMIT_REACHED` and bounded tombstone eviction remain visible, while local filesystem bookkeeping, retries, and filters are intentionally silent. Operational failures continue to use the `error` event.
