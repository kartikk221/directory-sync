...
Routine public logs are limited to `MANIFEST`, `WEBSOCKET`, `UPLOAD`, and `DOWNLOAD`. For every file transferred, its source emits `UPLOAD` start and completion logs while its destination emits matching `DOWNLOAD` start and completion logs. Manifest exchanges and WebSocket connections, reconnections, and disconnections are also logged. Exceptional conditions such as `SIZE_LIMIT_REACHED` and bounded tombstone eviction remain visible, while local filesystem bookkeeping, retries, filters, and hard-sync passes are intentionally silent. Operational failures continue to use the `error` event.
